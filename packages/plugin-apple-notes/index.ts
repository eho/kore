...
export interface AppleNotesPluginOverrides {
  /** Override syncNotes for testing — prevents real Apple Notes DB access */
  _syncNotesFn?: SyncNotesFn;
}

export function createAppleNotesPlugin(overrides?: AppleNotesPluginOverrides): KorePlugin {
  ...
  return {
    ...
    async start(d: PluginStartDeps) {
      ...
      const opts: SyncLoopOpts = {
        ...
        folderBlocklist,
        dbDir,
        ...(overrides?._syncNotesFn ? { _syncNotesFn: overrides._syncNotesFn } : {}),
      };
...
export default createAppleNotesPlugin();
