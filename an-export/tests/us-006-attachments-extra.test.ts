...
describe('Attachment Edge Cases', () => {
  ...
  test('InternalLink returns empty if ZTOKENCONTENTIDENTIFIER missing', async () => {
    ...
    const resolve = createAttachmentResolver({ db, entityKeys, accountPath: tempDir, exportDest: tempDir });
  ...
  test('Scan returns (empty scan) if ZHEXDATA missing', async () => {
    db.run(`INSERT INTO ziccloudsyncingobject (Z_PK, Z_ENT, ZIDENTIFIER) VALUES (1, 4, 'scan1')`);
    const resolve = createAttachmentResolver({ db, entityKeys, accountPath: tempDir, exportDest: tempDir });
    expect(await resolve({ attachmentIdentifier: 'scan1', typeUti: ANAttachmentUTI.Scan })).toBe('**(empty scan)**');
  });
  ...
  test('ModifiedScan returns copied file link with handwriting summary', async () => {
    ...
    const resolve = createAttachmentResolver({ db, entityKeys, accountPath: tempDir, exportDest, includeHandwriting: true });
